...
**바로 할 것**: 합류 모달 3종(pop 30/90/180)·대목수 환급·조력 이벤트 모달은 라이브 UI 확인됨. **남은 실효 검증**: 노상인 흥정 시작 +15·수입 +10%의 *실효*는 아직 이전 세션 결정론 API 검증만(합류 모달 perk 표기까지만 UI). 원하면 마을 이동→흥정으로 라이브 확인.
...
1. ~~인구 30↑ 라이브 스모크~~ **완료** (2026-08-01, 디버그 시드). 합류 모달 3종·대목수 환급 UI 확인, 조력 이벤트 모달 미표시 버그 수정. 남은 것: 노상인 흥정 시작 +15·수입 +10%의 *실효* 라이브 확인(현재 API 검증만).
