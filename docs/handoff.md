...
2. ~~**밸런스 재점검**~~ **1차 완료** (2026-08-04). 시뮬(`game-data.ts` 파싱) 진단: 회수기간 6~10일로 눈덩이 빠름, **tier3(marble·bronze·stainedglass)가 전부 자가 생산 가능해 병목 소실**(유일 실질 병목=relic 3개, 엔딩은 안 막음), 중반 골드 과잉으로 흥정 루프 무력화 리스크. **적용: tier1 가격 ×1.30**(economy.ts, 엔딩 18→26일·눈덩이 ~45%↓). 상세 `context-notes.md`(2026-08-04). **남은 후속(보류)**: tier3 자가 생산이 tier3 긴장 없애는 구조 이슈(생산율↓/책레벨 게이팅) — 마감 후 검토.
