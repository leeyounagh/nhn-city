...
- **노상인 흥정 시작 +15 실효 (라이브, 2026-08-01 추가)**: 마을 이동→신규 상인(삼나무 리나)과 실제 `/api/haggle` 흥정을 pop 30 vs pop 180에서 동일 발언으로 비교 → **시작 호감도 23 → 38 (+15)** 확인. 서버 시드 경로(disposition undefined 첫 턴)로 검증. 이전 세션 API 검증(23→38)과 일치. (디버그: `__seedPop`은 merchantMemory 초기화로 매번 신규 상인 상태 보장, `__travelTo`/`__startHaggle`/`__sendUtterance` 노출로 마을 nav clip-path 우회 — 검증 후 제거.)
- (미검증 잔여: 수입 +10%의 *실효*는 이전 세션 API 검증 유지.)
...
- **배경**: 압축·터치팬 이후에도 사용자가 "맵이 아직 좁다". GPT가 전면 재설계(전용 배치모드+확인버튼)를 제안했으나, ①탭-배치가 이미 됨 ②핀치/팬 이미 있음 ③온보딩 코치가 레이아웃에 강결합 → D-2에 회귀 위험. **접이식 팔레트 시트**(배치모드의 실용형)로 합의.
- **팔레트 터치 드래그**(`IsoCityMap`): `startBuildingDrag`·`stripPointerDown`의 `pointerType!=="mouse"` 가드만 제거하면, 기존 mouse용 방향 판정 로직(`scrollDragRef`: 가로=`scrollLeft` 수동 스크롤 / 세로=`dragRef` 건물 드래그)이 터치에 그대로 동작. 카드 `touch-none` 유지가 전제(브라우저 네이티브 스크롤이 수동 처리와 겹치지 않게). 탭-배치는 `moved` 플래그로 계속 구분돼 회귀 없음. 보드 터치팬과 무충돌(드래그는 카드에서 시작 → board `startPan` 미발화 → pan effect의 `pointersRef` 비어 no-op).
- **접이식 시트**(`IsoCityMap`): `buildOpen` state + `paletteVisible = buildOpen || forcePaletteOpen`. 렌더는 **CSS 게이팅 한 벌**(중복 컴포넌트 없음) — 핸들 버튼 `sm:hidden`, 본체 `className={paletteVisible ? "block" : "hidden sm:block"}`. 즉 데스크톱은 `paletteVisible`와 무관하게 항상 `block`(회귀 0), 모바일은 열렸을 때만. `clickBuilding`에서 `if(!forcePaletteOpen) setBuildOpen(false)`로 **고르면 자동 접힘**(데스크톱은 무영향). 인벤토리는 단순화 위해 보드 위 상시 유지(시트엔 팔레트만). 실측 보드 접힘 시 **282px**(펼치면 90px — 고를 땐 시트/놓을 땐 맵).
- **온보딩 결합점은 딱 1곳**: coach `mission-palette-hut`(=오두막 카드)만 팔레트가 열려 있어야 함. `Game.tsx`에서 `forcePaletteOpen={engine.mission?.objective.coach === "mission-palette-hut"}` 전달 → 그 미션 단계에선 시트 강제 오픈. `place`(카드 선택)→건물 배치되면 mission이 `fill`(coach `mission-hut`=보드 위 건물)로 넘어가며 force 해제→시트 접힘→맵 최대. 자연스러운 흐름. `data-coach` 전수 확인 결과 인벤토리/자재 칩을 가리키는 코치는 없어 인벤토리 상시노출 여부는 온보딩과 무관.

## 모바일 월드맵 — 상단 정렬 + 화면 꽉 채우기 (2026-08-08)
...
## 아이소맵 리사이즈 카메라 재정렬 (2026-08-08, QA 후속)
- **증상**: 뷰포트 크기 변경(모바일 회전·창 리사이즈·팔레트 시트 열고닫기) 시 카메라 pan이 최초 1회만 (0,0) 중앙정렬돼, 배치한 건물이 화면 밖으로 벗어날 수 있음.
- **수정**(`useIsoCamera.ts`): `prevViewportRef`로 직전 뷰포트를 추적하고, 뷰포트 변경 시 **중심 고정 보정** `newPan = pan + (Δ뷰포트)/2` 적용. 즉 "화면 중앙에 있던 월드점"을 계속 중앙에 유지 → 보던 영역이 그대로 유지되고 콘텐츠가 사라지지 않음. 최초 1회는 기존대로 (0,0)을 중앙에.
- **검증(playwright)**: 같은 레이아웃 리사이즈(폭 동일·높이만) 시 타일 오프셋 X·Y 정확 보존(43,22→43,22). 레이아웃 모드 전환(데스크톱↔모바일, 팔레트 리플로우로 보드 높이 비선형 변화) 시엔 오프셋 완전 보존은 불가하나 콘텐츠가 화면 안에 유지됨(on_screen). tsc·eslint 그린.
- **부수 효과(이득)**: 접이식 팔레트 시트를 열고닫아 보드 높이가 바뀔 때도 맵 콘텐츠가 튀지 않고 중심 유지.

## 초반 템포 — 오두막 단일 자재화 (2026-08-08, QA 후속 밸런스)
- **문제**: 첫 건물 오두막 = `나무5+돌3`. 나무는 삼목골(1일)·돌은 무쇠고개(2일) → 첫 완공까지 두 마을 왕복 강제, 초반 훅이 늦음.
- **변경**(`game-data.ts`): `hut.requires` `{ wood:5, stone:3 }`→`{ wood:5 }`. 첫 완공 = 최근접 1마을 1회 왕복 + 흥정 1회. **흥정 AI는 크리티컬 패스 유지**(나무를 흥정으로 사와야 함). 둘째 건물부터 멀티 자재 유지(쇼핑 재미 학습).
...
## 데모 훅 재조정 — 상인 성향 추리를 첫 경험으로 (2026-08-09, D-1)
- **문제(GPT 피드백+실측)**: 킬러 콘텐츠(상인 성향·약점 추리)가 책 Lv2/Lv3에 잠겨 있어 5분 심사에선 안 보임. `BOOK_XP_THRESHOLDS=[0,30,60]` → 성향 Lv2(건물 3채)·약점 Lv3(6채). 첫 경험은 "나무 사러 가라→오두막" 건설 심부름. → 재미의 본체가 문 뒤에 잠김.
- **판단**: D-1이라 시스템 걷어내기(리빌드)는 리스크↑. **재미를 앞으로 끌어내는(unlock+re-sequence)** 게 최적. 리빌드 아님.
- **① 책 Lv2 시작**(`game-data.ts BOOK_XP_THRESHOLDS [0,30,60]→[0,0,8]`): xp0에서 Lv2(성향 노출), 첫 건물(xp≥8)에서 Lv3(약점). 첫 흥정부터 "이 상인 실리형" 같은 추리 단서가 보임. 부수효과: tier2 건물 즉시 배치·relic 첫 건물 후 — 데모엔 오히려 할 게 많아 이득.
- **② 힌트 넛지**(`missions.ts` go-buy): "흥정 땐 상인의 성향(책 분석)을 살펴 약점을 찌르면 값이 내려간다" 추가 — 추리 사용 유도.
- **검증(playwright)**: 새 게임 책 Lv.2 시작 확인, 첫 흥정 데스크톱서 성격+성향 노출(약점 Lv3 잠금), 콘솔 0.
- **미적용(후순위)**: ③ 약점 적중 피드백(호감도 급등 시 "정곡" 연출), ④ 첫 화면 HUD 절제 — 시간 남으면.

## ③ 약점 적중 피드백 — 추리 성공의 쾌감 (2026-08-09, D-1)
- **목적**: 성향 약점을 찌를 때 "정곡을 찔렀다!" 연출로 추리 성공을 legible하게(GPT 피드백의 "한 번 더 해보고 싶은" 훅).
- **서버**(`economy.categoryEffect` export + `/api/haggle`): 클램프 전 원 효과(`EFFECT[category][profile]`)로 판정 — `weaknessHit = rawEffect>=16`(성향별 최대 반응 16~24), `backfire = rawEffect<=-6`(협박 등). 응답에 두 플래그 추가. (클라의 dispositionDelta는 클램프돼 캡 근처서 왜곡되므로 서버 raw 기준으로 판정.)
...
## 설계도(건축·장식 해금) 안내 추가 + 모바일 점검 (2026-08-09)
- **바닥/장식 메커니즘**: floor_* 등 장식은 `requires:{}`(자재 0·즉시완공·수입0). 배치 해금은 자재가 아니라 **「대건축가의 설계도」(blueprint) 보유**(`hasBlueprint = inventory.blueprint>0`, 소모 안 됨·1개면 전체 해금). 설계도는 물물교환 전용 희귀템(구매 불가, 책 Lv3). blueprint는 어떤 건물의 `requires`에도 없어 도감 범례(usedSpecials)엔 안 잡힘 → 안내를 팔레트에 둠.
- **안내 추가**(`BuildingPalette`): 탭이 arch/deco일 때 라벨 아래 "🔎 「대건축가의 설계도」 보유 시 해금 — 물물교환으로만 얻는 희귀템(1개면 전체 해금)" 노출(호버 없이). 잠긴 장식 카드 title 툴팁도 획득 경로 추가.
- **모바일 점검(390px)**: 설계도 안내 2줄 정상, 접이식 시트·팔레트·카테고리 한줄스크롤·바닥 카드("설계도 필요")·2줄 푸터 이상 없음. (도감 범례는 데스크톱 검증됨; 모바일 재확인은 Next dev-tools "N" 오버레이가 푸터 책버튼을 가려 자동클릭만 막힘 — 배포본엔 없는 개발 전용 오버레이라 제품 무관.)
